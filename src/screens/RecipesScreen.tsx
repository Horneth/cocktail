...
export function RecipesScreen() {
  ...
  return <div className={styles.screen}>
    ...
    {list.length === 0 ? <div className={styles.empty}><h2>{cocktails.length === 0 ? 'No drinks yet' : 'Nothing matches'}</h2><p>{cocktails.length === 0 ? 'Tap Recipe to type your first one in.' : 'Try another spirit, or go back to All.'}</p></div> : <div className={styles.grid}>{list.map((recipe) => <Link key={recipe.id} className={styles.card} to={`/recipe/${recipe.id}`}><div className={styles.photo}><RecipeImage image={recipe.image} size="card" sizes="(max-width: 480px) 45vw, 200px" className={styles.photoImg} fallback={showCardArt(recipe)} /></div><div className={styles.cardFoot}><h2>{recipe.name}</h2><span className={`${styles.status} ${have.size && makeableIds([recipe], byId, have, assumeStaples).has(recipe.id) ? styles.ready : styles.missing}`}><i />{have.size ? (makeableIds([recipe], byId, have, assumeStaples).has(recipe.id) ? 'Ready' : 'Missing') : 'Ready'}</span></div></Link>)}</div>}
