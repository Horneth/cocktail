...
describe('functions/shared/curatedKeys.mjs matches classics.json', () => {
  it('contains exactly the pool keys of the curated classics', async () => {
    const classics: { name: string }[] = JSON.parse(
      readFileSync(resolve(root, 'scripts/classics.json'), 'utf8'),
    )
    const expected = new Set(classics.map((c) => poolKeyForName(c.name)).filter(Boolean))
    const { CURATED_KEYS } = await import('../../functions/shared/curatedKeys.mjs')
    expect(CURATED_KEYS.size).toBe(expected.size)
    for (const key of expected) expect(CURATED_KEYS.has(key)).toBe(true)
  })

  it('was produced by the generator (never hand-edited)', () => {
    const content = readFileSync(resolve(root, 'functions/shared/curatedKeys.mjs'), 'utf8')
    expect(content).toContain('GENERATED by scripts/sync-curated-keys.mjs')
  })
})
